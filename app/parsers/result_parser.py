...
from ..utils.data_utils import extract_text_and_tables_from_pdf
from ..utils.page_fetcher import fetch_page, fetch_pdf
...
class ResultParser:
    ...
    def __init__(self, result_page, version=None, url=None):
    ...
    async def get_other_versions(self):
        """
        ...
        for version in other_versions:
            ...
            response = await fetch_page(version["href"])
            response = await response.text()
            soup = BeautifulSoup(response, "html.parser")
            version_text = version.text.strip().replace("\n", " ")
            result = ResultParser(
                soup, url=version["href"], version=version_text)
            result = await result.parse()
    ...
    async def get_pdf_zip(self, pdf_link):
        """
        ...
        """
        response = await fetch_pdf(pdf_link)
        response = await response.read()
        return response

    async def get_pdf_data(self):
        """
        ...
        zip_bytes = await self.get_pdf_zip(self.pdf_link)
